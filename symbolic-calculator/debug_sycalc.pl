...
deb_red_frac(I, F, RES):- red_frac(F, R), write(I), write(F), write(' = '), output_text(R, RES).

sum_deb(_, A, B, RES):- eval_sum(A, B, R), RES == R, !.
sum_deb(I, A, B, RES):- eval_sum(A, B, R), write(I), write(A), write(' + '), write(B), write(' = '), output_text(R, RES).

sub_deb(_, A, B, RES):- eval_sub(A, B, R), R == RES, !.
sub_deb(I, A, B, RES):- eval_sub(A, B, R), write(I), write(A), write(' - '), write(B), write(' = '), output_text(R, RES).

prod_deb(_, A, B, RES):- eval_prod(A, B, R), R == RES, !.
prod_deb(I, A, B, RES):- eval_sub(A, B, R), write(I), write(A), write(' * '), write(B), write(' = '), output_text(R, RES).

power_deb(_, A, B, RES):- eval_pow(A, B, R), R == RES, !.
power_deb(I, A, B, RES):- eval_pow(A, B, R), write(I), write(A), write('^'), write(B), write(' = '), output_text(R, RES).

arithm_deb(_, E, RES):- arithmetic_eval(E, R), R == RES, !.
arithm_deb(I, E, RES):- arithmetic_eval(E, R), write(I), write(E), write(' = '), output_text(R, RES).

debug_numeric:-
	write('-- NUMERICAL DEBUG --'), nl,
	...
	sum_deb(' 5) ', 0, 5, 5),
	sum_deb(' 6) ', 1/2, 1/2, 1),
	sum_deb(' 7) ', 1/2, 2/2, 3/2),
	sum_deb(' 8) ', 1/2, 1, 3/2),
	sum_deb(' 9) ', 1, 1/2, 3/2),
	sum_deb('10) ', 2/2, 2/2, 2),
	sum_deb('11) ', 2/2, 3/2, 5/2),
	sum_deb('12) ', 3/2, 1, 5/2),
	...
	sub_deb(' 5) ', 0, 5, -5),
	sub_deb(' 6) ', 1/2, 1/2, 0),
	sub_deb(' 7) ', 1/2, 2/2, -1/2),
	sub_deb(' 8) ', 1/2, 1, -1/2),
	sub_deb(' 9) ', 1, 1/2, 1/2),
	sub_deb('10) ', 2/2, 2/2, 0),
	sub_deb('11) ', 2/2, 3/2, -1/2),
	sub_deb('12) ', 3/2, 1, 1/2),
	...
	prod_deb(' 4) ', 0, 1, 0),
	prod_deb(' 5) ', 0, 5, 0),
	prod_deb(' 6) ', 1/2, 1/2, 1/4),
	prod_deb(' 7) ', 1/2, 2/2, 1/2),
	prod_deb(' 8) ', 1/2, 1, 1/2),
	prod_deb(' 9) ', 1, 1/2, 1/2),
	prod_deb('10) ', 2/2, 2/2, 1),
	prod_deb('11) ', 2/2, 3/2, 3/2),
	prod_deb('12) ', 3/2, 1, 3/2),
	...
	power_deb(' 4) ', 0, 1, 0),
	power_deb(' 5) ', 0, 5, 0),
	power_deb(' 6) ', 1/2, 2, 1/4),
	power_deb(' 7) ', 1/2, 1, 1/2),
	power_deb(' 8) ', 2, 3, 8),
	power_deb(' 9) ', 2/2, 2, 1),
	power_deb('10) ', 2/2, 3, 1),
	power_deb('11) ', 3/2, 1, 3/2),
	power_deb('12) ', 3/2, 2, 9/4),
	
	write('* ARITHMETIC EXPRESSIONS'), nl,
	arithm_deb(' 1) ', 3 + 3, 6),
	arithm_deb(' 2) ', 3 - 4, -1),
	arithm_deb(' 3) ', 3^4, 81),
	arithm_deb(' 4) ', 2^2^2^2, 256),
	arithm_deb(' 5) ', 2^2^2^2^2, 65536),
	arithm_deb(' 6) ', 2^(1 + 1)^2, 16),
	arithm_deb(' 7) ', 2^2^(2 + 1 - 1)^2^2, 65536),
	arithm_deb(' 8) ', (1/2)^2 + 3/4, 1),
	arithm_deb(' 9) ', (1/2)^0, 1),
	arithm_deb('10) ', (1/2)^0 + 3/4, 7/4),
	arithm_deb('11) ', 1 - (1 + 1), -1),
	arithm_deb('12) ', 1/2, 1/2),
...
deb_red_mon(I, M, RES):- red_monomial(M, R), write(I), write(M), write(' = '), output_text(R, RES).

deb_mon_sum(_, M1, M2, RES):- mon_sum(M1, M2, S), S == RES, !.
deb_mon_sum(I, M1, M2, RES):- mon_sum(M1, M2, S), write(I), write(M1), write(' + '), write(M2), write(' = '), output_text(S, RES).

deb_mon_sub(_, M1, M2, RES):- mon_sub(M1, M2, S), S == RES, !.
deb_mon_sub(I, M1, M2, RES):- mon_sub(M1, M2, S), write(I), write(M1), write(' - '), write(M2), write(' = '), output_text(S, RES).
...
deb_poly_sum(I, P, RES):- polynomial_sum(P, R), write(I), write(P), write(' = '), output_text(R, RES).
...
deb_poly_eval(I, E, RES):- polynomial_evaluation(E, R), write(I), write(E), write(' = '), output_text(R, RES).

debug_polynomials:-
	write('-- POLYNOMIALS DEBUG --'), nl,
