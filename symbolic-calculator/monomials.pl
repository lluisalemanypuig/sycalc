...
monomial_components(- (M), NC, V, E):- monomial_components(M, C, V, E), fraction(C), neg_fraction(C, NC), !.
monomial_components(- (M), NC, V, E):- monomial_components(M, C, V, E), NC is -C, !.
monomial_components(C*X^E, CE, X, EE):- arithmetic_eval(C, CE), arithmetic_eval(E, EE), !.
monomial_components(X^E, 1, X, EE):- arithmetic_eval(E, EE), !.
monomial_components(C*X, CE, X, 1):- arithmetic_eval(C, CE), !.
monomial_components(C, CE, _, 0):- arithmetic_eval(C, CE), !.
monomial_components(X, 1, X, 1):- not(expr(X)).

% Monomial definition

monomial(M):- monomial_components(M, C, _, E), arithmetic_eval(C, CE), arithmetic_eval(E, EE).

monomial_neg(M, CN*V^E):- monomial_components(M, C, V, E), CN is -C.
...
monomial_reduced_components(C, V, E, R):- arithmetic_eval(C, CE), arithmetic_eval(E, EE), monomial_reduced__(CE, V, EE, R).
...
pretty_monomials_prod(CX, _,  0, CY, _,  0, C):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, _,  0, CY, Y,  1, C*Y):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, X,  1, CY, _,  0, C*X):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, X,  1, CY, Y,  1, C*X*Y):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, X,  1, CY, Y, EY, C*X*(Y^EY)):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, X, EX, CY, Y,  1, C*(X^EX)*Y):- arithmetic_eval(CX*CY, C), !.
pretty_monomials_prod(CX, X, EX, CY, Y, EY, C*(X^EX)*(Y^EY)):- arithmetic_eval(CX*CY, C).
...
monomial_sum(M1, -(M2), R):- monomial_sub(M1, M2, R), !.
...
monomial_sum(M1, M2, M3):-
	...
	E1 == E2,
	arithmetic_eval(C1 + C2, S), !, monomial_reduced_components(S, V1, E1, M3).
...
monomial_prod(M1, M2, M3):-
	...
	arithmetic_eval(C1*C2, C), arithmetic_eval(E1 + E2, E), monomial_reduced_components(C, V1, E, M3).
...
debug_monomials:-
	...
	nl, write('- MONOMIAL REDUCTION -'), nl, nl,
