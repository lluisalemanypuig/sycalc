...
list_polynomial_sum_list(P1, P2, R):-
	concat(P1, P2, P), list_red_list_polynomial(P, R).
...
polynomial_prod(P1, P2, P):-
	...
	list_polynomial_prod_list(L1, L2, MON_PROD),
	list_polynomial(MON_PROD, P).
...
polynomial_evaluation_list(Q1 * Q2, R):-
	...
	list_red_list_polynomial(PROD, R), !.
...
polynomial_evaluation(P, R):-
	polynomial_evaluation_list(P, L), list_polynomial(L, R).
...
expanded_polynomial_evaluation(VAL, P, E):-
	polynomial_monomials(P, MS), map(monomial_evaluation(VAL), MS, R),
